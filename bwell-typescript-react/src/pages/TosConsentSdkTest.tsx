...
import { BWellSDK, GetConsentsRequest } from "@icanbwell/bwell-sdk-ts";
...
const TosConsentSdkTest = () => {
  ...
  const runGetTosConsents = async () => {
    ...
    try {
      ...
      appendLog('Calling sdk.user.getConsents(new GetConsentsRequest({ category: "tos" }))');
      const result: any = await sdk.user.getConsents(new GetConsentsRequest({ category: "TOS" }));
    ...
    <Container>
      ...
      <Box display="flex" flexDirection="column" gap={2} mb={2}>
        ...
        <Box display="flex" gap={2} flexWrap="wrap" alignItems="center">
          <Button variant="contained" onClick={runGetTosConsents} disabled={!canRun}>
            {loading ? "Running..." : "Get TOS Consents"}
          </Button>
      ...
      {consents.length > 0 && (
        <Box mb={2}>
          <Alert severity="success">Found {consents.length} TOS consent(s).</Alert>
        </Box>
      )}
...
export default TosConsentSdkTest;
